...
ngOnInit() {
    ...
    listPermission.forEach((permission:any) => {
      if(permission==="Admin"){
        ...
        return true;
      ...
      else if(permission === "Permission_Create"){
            this.showbtCreate = true;
         ...
         else if(permission === "Permission_DeleteMutiple"){
          this.showbtDeleteMultiple = true;
         }
      return false;
    });
...
getPermissions() {
  debugger;
  this.crudService.getAll('Permission/GetAll_Role').subscribe(
    (result) => {
        this.roles = result;
    },
    (error) => {
...
editPermission(permission: Permission) {
  debugger
  this.crudService.getById("Permission/GetById",permission.PermissionId).subscribe(
    (data:any)=>{
...
if(this.permission.Name?.trim()){
    if(this.permission.PermissionId){
        ...
        this.crudService.updateItem('Permission/Update', this.permission).subscribe(
            result =>{
...
this.permission.PermissionRoles?.forEach((permissionRole:any) => {
      ...
      this.crudService.createItem('Permission/Create',this.permission).subscribe(
        result=>{
          this.permissions.forEach(() => {
            this.permission.PermissionRoles?.forEach((permissionRole:any) => {
              const roleId = permissionRole.RoleId;
...
this.crudService.deleteItem('Permission/Delete',permission.PermissionId).subscribe(
          data => {
...
deleteSelectedPermissions() {
  ...
  this.crudService.deleteMultipleItem("Permission/DeleteMultiple",this.selectedPermissions).subscribe(
    data=>{
      if(data){
