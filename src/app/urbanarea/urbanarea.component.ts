...
export class UrbanareaComponent {
  ...
  constructor(private shareData:ShareDataService,private crudService:CrudService, private messageService: MessageService){}
    ...
    if(this.urbanarea.UrbanName?.trim()){
        if(this.urbanarea.UrbanId){
            this.crudService.updateItem('UrbanArea/Update', this.urbanarea).subscribe(
                result =>{
   ...
   this.urbanarea.Population = 0;
          this.crudService.createItem('UrbanArea/Create',this.urbanarea).subscribe(
            (result:any)=>{
    ...
    deleteUrbanArea(urbanarea: UrbanArea){
      if(confirm(`Bạn có muốn xóa khu đô thị ${urbanarea.UrbanName} không?`)){
            this.crudService.deleteItem('UrbanArea/Delete',urbanarea.UrbanId).subscribe(
              data => {
                if(data){
    ...
    this.crudService.deleteMultipleItem("UrbanArea/DeleteMultiple",this.selectedUrbanareas).subscribe(
      data=>{
        if(data){
